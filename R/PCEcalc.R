...
calcPCE <- function(age, race, sex, tc, hdl, sbp, bptx, smoking, dm, white=c(0,"w","white"),
  ...
  pce[ww] <- 29.18 +
    ...
    (3.149 * log(age[ww]) * log(hdl[ww])) + (2.019 * log(sbp[ww]) * bprx[ww]) +
    (1.957 * log(sbp[ww]) * as.numeric(bprx[ww]==0)) + (7.574 * smoke[ww]) +
    (-1.665 * log(age[ww]) * smoke[ww]) + (0.661 * diab[ww])
  ...
  pce[bw] <- -86.61 +
    ...
    (4.475 * log(age[bw]) * log(hdl[bw])) + (29.291 * log(sbp[bw]) * bprx[bw]) +
    (-6.432 * log(age[bw]) * log(sbp[bw]) * bprx[bw]) +
    (27.820 * log(sbp[bw]) * as.numeric(bprx[bw]==0)) +
    (-6.087 * log(age[bw]) * log(sbp[bw]) * as.numeric(bprx[bw]==0)) +
  ...
  pce[wm] <- -61.18 +
    ...
    (-7.990 * log(hdl[wm])) + (1.769 * log(age[wm]) * log(hdl[wm])) + (1.797 * log(sbp[wm]) * bprx[wm]) +
    (1.764 * log(sbp[wm]) * as.numeric(bprx[wm]==0)) + (7.837 * smoke[wm]) +
    (-1.795 * log(age[wm]) * smoke[wm]) + (0.658 * diab[wm])
